...
class Kpop3:
    def __init__(self, Kpop3):
        ...
        self.title = tkinter.Label(self.Kpop3, text=" ♡ nextlevel ♡ ", foreground="#d375ff", font=self.titleFont)
        self.title.place(x=280, y=40)

        # 불러오기
        self.info = open("kpop3.txt", "r", encoding="utf-8")
        addY = 150
        while True:
            ...
            for self.line in self.infoData:
                self.title = tkinter.Label(self.Kpop3, text=self.line, foreground="#d375ff")
        ...
        self.homeBtn = tkinter.Button(self.Kpop3, text="home", image=self.home, command=self.moveHomeBtn, repeatdelay=20)
        self.homeBtn.place(x=50, y=400)
        self.back = tkinter.PhotoImage(file="image/back.gif")
        self.backBtn = tkinter.Button(self.Kpop3, text="home", image=self.back, command=self.moveBackBtn, repeatdelay=20)
        self.backBtn.place(x=710, y=400)

    # home 가기
    def moveHomeBtn(self):
        game.Start(self.Kpop3)

    # 뒤로가기
    def moveBackBtn(self):
        kpop.Kpop(self.Kpop3)