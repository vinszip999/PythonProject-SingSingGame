...
class Ballade3:
    def __init__(self, Ballade3):
        self.Ballade3 = Ballade3
        ...
        self.title = tkinter.Label(self.Ballade3, text=" ♡ 좋니 ♡ ", foreground="#d375ff", font=self.titleFont)
        self.title.place(x=315, y=40)

        # 불러오기
        self.info = open("ballade3.txt", "r", encoding="utf-8")
        addY = 150
        while True:
            ...
            for self.line in self.infoData:
                self.title = tkinter.Label(self.Ballade3, text=self.line, foreground="#d375ff")
        ...
        self.homeBtn = tkinter.Button(self.Ballade3, text="home", image=self.home, command=self.moveHomeBtn, repeatdelay=20)
        self.homeBtn.place(x=50, y=400)
        self.back = tkinter.PhotoImage(file="image/back.gif")
        self.backBtn = tkinter.Button(self.Ballade3, text="home", image=self.back, command=self.moveBackBtn, repeatdelay=20)
        self.backBtn.place(x=710, y=400)

    # home 가기
    def moveHomeBtn(self):
        game.Start(self.Ballade3)

    # 뒤로가기
    def moveBackBtn(self):
        ballade.Ballade(self.Ballade3)